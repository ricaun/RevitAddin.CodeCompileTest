    ...
    public class App : IExternalApplication
    {
        ...
        public Result OnStartup(UIControlledApplication application)
        {
            ...
            foreach (var item in ribbonPanel.GetRibbonItems())
            {
                ...
                QuickAccessToolBarAdd(item.GetRibbonItem());
            }
        ...
        public Result OnShutdown(UIControlledApplication application)
        {
            ...
            foreach (var item in ribbonPanel.GetRibbonItems())
            {
                QuickAccessToolBarRemove(item.GetRibbonItem());
            }
            ribbonPanel.Remove();
        ...
        public T QuickAccessToolBarRemove<T>(T ribbonItem) where T : Autodesk.Windows.RibbonItem
        {
            var ri = Autodesk.Windows.ComponentManager.QuickAccessToolBar.Items
                .FirstOrDefault(e => e.Id == ribbonItem.Id);

            if (ri != null)
                Autodesk.Windows.ComponentManager.QuickAccessToolBar.Items.Remove(ri);
        ...
        /// <returns></returns>
        public T QuickAccessToolBarAdd<T>(T ribbonItem) where T : Autodesk.Windows.RibbonItem
        {
            var ri = Autodesk.Windows.ComponentManager.QuickAccessToolBar.Items
                .FirstOrDefault(e => e.Id == ribbonItem.Id);

            if (ri == null)
                Autodesk.Windows.ComponentManager.QuickAccessToolBar.Items.Add(ribbonItem);
